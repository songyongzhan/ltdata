...
class Validate {
      ...
      if (FALSE === $result) {
        ...
        if (isset($msg[$i])) {
          ...
          if (is_string($message) && strpos($message, '{%') === 0) {
            $message = Validate::get(substr($message, 2, -1));
          }
        } else {
  ...
  protected function getRuleMsg($attribute, $title, $type, $rule) {
    if (isset($this->message[$attribute . '.' . $type])) {
      ...
      $msg = $title . Validate::get('not conform to the rules');
    }

    if (is_string($msg) && 0 === strpos($msg, '{%')) {
      $msg = Validate::get(substr($msg, 2, -1));
    } elseif (Validate::has($msg)) {
  ...
  public static function has($name) {
    ...
    return isset(self::$langMsg[strtolower($name)]);
  }

  public static function get($name = NULL, $vars = []) {

    $key = strtolower($name);
    $value = isset(self::$langMsg[$key]) ? self::$langMsg[$key] : $name;
